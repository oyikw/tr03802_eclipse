...
<body>
	...
	%>
		<tr>
			<td><%= p.getName() %></td>
			<td>￥<%= p.getPrive() %></td>
			<td><input style="text-align: center; width: 50px"
				value="<%= p.getCount() %>">
				<button onclick="update(this,<%= p.getPid() %>)">更新</button></td>
			<td>￥<%= p.getTotal() %></td>
			<td><a
				href="javascript:if(confirm('您真的要删除吗？')) location='remove.jsp?pid=<%= p.getPid() %>';">删除</a></td>
		</tr>
		<% 		
		}
	%>
